...
def load_whisper_model():
    ...
    try:
        ...
        print(f"加载对齐模型失败：{e}")
        print("请确保网络连接正常，并且 'model_name' 和 'language_code' 正确。")
        print("如果仍有问题，尝试手动下载模型文件到缓存目录。")
        exit()
...
def text_algn(model_a, metadata, device, audio_path, gt_text):
    ...
    if not os.path.exists(audio_file_path):
        ...
        print("请将 'audio_file_path' 变量替换为你的实际音频文件路径。")
        exit()
    print(f"正在加载音频文件: {audio_file_path} 并进行VAD...")
    ...
    print(f"准备对齐文本: '{ground_truth_text}' (总时长: {audio_duration:.2f} 秒)")
